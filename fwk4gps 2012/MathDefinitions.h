...
Quaternion::Quaternion(float wX, float wY, float wZ)
{
   float   
   roll     = DEGTORAD(wX) / 2.0f, 
   pitch    = DEGTORAD(wY) / 2.0f,
   yaw      = DEGTORAD(wZ) / 2.0f,
   ...
   sinYawcosPitch    = sinYaw * cosPitch;
    ...
    x = sinRoll * cosYawcosPitch     -    cosRoll * sinYawsinPitch;
    y = cosRoll * sinPitch * cosYaw  +    sinRoll * cosPitch * sinYaw;
    z = cosRoll * cosPitch * sinYaw  -    sinRoll * sinPitch * cosYaw;
    w = cosRoll * cosYawcosPitch     +    sinRoll * sinYawsinPitch;

    normalize();
}

// From axis angle represent
Quaternion::Quaternion(Vector v, float w)
{
